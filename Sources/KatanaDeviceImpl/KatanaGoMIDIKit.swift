...
public actor KatanaGoMIDIKit: KatanaDevice {
  ...
  private let memoryBankCache = MemoryBankCache()
...
private class MemoryBankCache {
  ...
  func update(address: UInt32, data: [UInt8]) {
  ...
  /// - Returns: The requested data if all bytes are present in the cache, otherwise `nil`.
  func get(address: UInt32, length: Int) -> [UInt8]? {
    ...
    for _ in 0..<length {
      ...
      guard let bank = hierarchy[byte1]?[byte2]?[byte3],
        ...
        // If any byte in the requested range is missing, return nil
        return nil
      }
      result.append(value)
  ...
  private func getOrCreateBank(byte1: UInt8, byte2: UInt8, byte3: UInt8) -> MemoryBank {
    // 1. Ensure the first level branch exists for byte1
    let branch1 = hierarchy[byte1] ?? [:]
    hierarchy[byte1] = branch1

    // 2. Ensure the second level branch exists for byte2
    var branch2 = branch1[byte2] ?? [:]

    // 3. Return the existing bank if it's already in the hierarchy
    if let existingBank = branch2[byte3] {
      return existingBank
    }

    // 4. Otherwise, create a new bank and wire it back into the hierarchy
